...
void insert_sjf_task_rb_tree(struct sjf_rq *rq, struct sched_sjf_entity *p)
{
	...
	while(*node!=NULL){
		...
		if(entry){
			ct += entry->sjf_bt;
			if(p->sjf_status == 0 && p->sjf_arr <= ct && p->sjf_bt < entry->sjf_bt){
				node=&parent->rb_left;
			}else{
...
static void enqueue_task_sjf(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if(p){
		...
		snprintf(msg_sjf,SJF_MSG_SIZE,"ENQ(cid%d:pid%d:at%d:bt%d)",ce->sjf_id,p->pid,ce->sjf_arr,ce->sjf_bt);
...
static void dequeue_task_sjf(struct rq *rq, struct task_struct *p, int sleep)
{
	...
	if(p){
		...
		if(1){
			snprintf(msg_sjf,SJF_MSG_SIZE,"DEQ(cid%d:pid%d:at%d:bt%d)",ce->sjf_id,p->pid,ce->sjf_arr, ce->sjf_bt);
